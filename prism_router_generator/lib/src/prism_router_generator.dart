...
final class PrismRouterGenerator extends GeneratorForAnnotation<PrismRouterConfig> {
  @override
  FutureOr<String> generateForAnnotatedElement(
    ...
    out.writeln(_headerComment('Generated by prism_router_generator. Do not edit.'));
    ...
    out.writeln('final List<PrismRouteDefinition> appRoutes = <PrismRouteDefinition>[');
    for (final r in routes) {
      out.writeln(
    ...
    out.writeln('  List<NavigatorObserver> observers = const <NavigatorObserver>[],');
    out.writeln(
  ...
  Future<List<_RouteInfo>> _collectRoutes(BuildStep buildStep) async {
    ...
    // Iterate all libraries in the current build; pick only those from this package.
    final libs = await buildStep.resolver.libraries.toList();
    for (final lib in libs) {
      final uri = lib.source.uri;
      // Consider only libraries from the current package. Depending on the
      // resolver, these can appear as `asset:` or `package:` URIs.
      final isInPackage = switch (uri.scheme) {
        'asset' => uri.path.startsWith('$package/'),
        'package' => uri.path.startsWith('$package/'),
        _ => false,
      };
  ...
  String _generatePageClass(_RouteInfo r) {
    ...
    final usableParams =
        params.where((p) => p.name != 'key').toList(growable: false);
    ...
    final argsMap = usableParams.isEmpty
        ? 'const <String, Object?>{}'
        : '<String, Object?>{${usableParams.map((p) => '${_q(p.name)}: ${p.name}').join(', ')}}';

    final childArgs = usableParams.map((p) => '${p.name}: ${p.name}').join(', ');
    final childExpr = childArgs.isEmpty
        ? '${r.screen.displayName}()'
        : '${r.screen.displayName}($childArgs)';

    final tagsExpr = r.tags.isEmpty
        ? 'null'
...
String _emitFromArgs(_RouteInfo r, List<ParameterElement> params) {
  ...
  if (patternEntries.isNotEmpty) {
    ...
    final okArgs =
        params.map((p) => p.isNamed ? '${p.name}: ${p.name}' : p.name).join(', ');
